    ...
    <form method="POST" action="/dashboard/videos/{{$video->slug}}" class="mb-5" enctype="multipart/form-data">
      @method('put')
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{old('title', $video->title)}}" required autofocus>
          @error('title')
        ...
        <div class="mb-3">
          <label for="category" class="form-label">Category Video</label>
            <select class="form-control" name="category_id">
              @foreach($categories as $category)
                @if(old('category_id', $video->category_id) == $category->id)
        ...
        <div class="mb-3">
          <label for="image" class="form-label">Video Thumbnail</label>
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control @error('video') is-invalid @enderror" id="title" name="video" value="{{old('video', $video->video)}}" required autofocus>
          @error('video')
            <div class="invalid-feedback">
              {{$message}}
            </div>
